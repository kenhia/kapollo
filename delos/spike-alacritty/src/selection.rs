//! S2 selection state machine over **content coordinates**, delegating range math to
//! `spike_support::coords`. Identical in spirit to S1 — proves the same pure model
//! drives a different grid backend (`alacritty_terminal`).
//!
//! Content coordinates: `(content_row, col)` where `content_row` is stable under
//! scrolling (see `main.rs` for the `top_row = BASE - display_offset` mapping).
