//! S3 selection state machine over **content coordinates** — here the content row is
//! a wezterm `StableRowIndex` (a true absolute row id), so the same pure model is even
//! more natural than in S1/S2. Logic is byte-for-byte identical to S1/S2 (same tests).
