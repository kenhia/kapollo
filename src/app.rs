...
impl App {
    ...
    pub fn run<B: Backend>(&mut self, terminal: &mut Terminal<B>) -> anyhow::Result<()> {
        while !self.should_quit {
            ...
            if event::poll(POLL_INTERVAL)? {
                match event::read()? {
                    Event::Key(key) if key.kind == KeyEventKind::Press => {
                        if self.passthrough {
                            // Forward keystrokes verbatim to the program (FR-018).
                            if let Some(bytes) = crate::ui::passthrough::encode_key(key) {
                    ...
                    // Forward resize to the PTY in every mode so full-screen
                    // programs reflow live (FR-017, FR-019, SC-008).
            ...
            self.drain_shell();
            // On returning from a full-screen program, repaint the split-pad UI
            // with the prior transcript intact (FR-020).
            ...
            match event {
                PtyEvent::Output(bytes) => {
                    ...
                    for boundary in boundaries {
                        if let Boundary::CommandEnd { exit_code } = boundary {
    ...
    fn on_key(&mut self, key: KeyEvent) {
        match (key.code, key.modifiers) {
            ...
            // PageUp/PageDown scroll the transcript independently (FR-014).
            (KeyCode::PageUp, _) => self.transcript.scroll_up(1),
            (KeyCode::PageDown, _) => self.transcript.scroll_down(1),
